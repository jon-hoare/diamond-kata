...
public class Diamond {
  ...
  public Diamond(String character) {
    ...
    int middleCharIndex = character.charAt(0) - 'A' + 1;
  ...
  private static String getDiamondLine(int i, int middleCharIndex) {
    // 'A' is a special case since it only appears once on the line
    if (i == 0) {
      String spaces = SPACE.repeat(middleCharIndex - 1);
      return spaces + "A" + spaces;
    }
    // All other chars appear twice
    String spacesAtEnds = SPACE.repeat(middleCharIndex - 1 - i);
